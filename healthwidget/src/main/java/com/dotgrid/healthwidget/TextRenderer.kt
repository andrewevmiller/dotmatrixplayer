 ...
 * `android:fontFamily` resource when `!context.isRestricted()`. In a widget
 * that test fails, so the attribute is skipped in silence - no exception,
 * nothing in the log, just the system face where the custom one should be.
 * Setting a `Typeface` from code does not help either: `TypefaceSpan` parcels
 * a family *name*, not a face, so it arrives in the launcher meaning nothing.
 *
 * Pixels are the only thing that survives the trip. We load the face in our
 * own process, where it resolves normally, draw the text, and send the result
 * as a bitmap.
 */
object TextRenderer {
     ...
     * vocabulary - digits, five metric names, "GOAL", a clock time. The wide
     * tile alone puts eleven bitmaps on screen at once and the two layouts
     * differ in size, so this is a couple of full repaints' worth rather than
     * a couple of labels'.
     */
    private val cache = object : LruCache<String, Bitmap>(48) {}

    /**
     * The unit rides beside a number two or three times its size, so it needs
     * the extra air to read as a label rather than as a smudge on the end of
     * the figure.
     */
    ...
    /*
     * fontRes names a role, not a default - see Typography. Every call site
     * states Typography.BODY, Typography.ACCENT, or (once, for the settings
     * headline) Typography.WORDMARK explicitly, the same way the sibling
     * widgets do, so a change to the brand doc is a change to one file rather
     * than a hunt through every call here.
     */
    /**
     ...
     * descents instead - which at 22sp against 9sp leaves the unit visibly
    ...
    fun renderReadout(
        ...
        val key = "readout|$value|$valueSizePx|$valueColor|$unit|$unitSizePx|$unitColor|$gapPx|$pillColor"
        cache.get(key)?.let { if (!it.isRecycled) return it }

        // A readout is a data readout: Body/Geist, always - see Typography.
        val valuePaint = paint(context, Typography.BODY, valueSizePx, 0f).apply { color = valueColor }
        val unitPaint = paint(context, Typography.BODY, unitSizePx, TRACKING_UNIT).apply { color = unitColor }
