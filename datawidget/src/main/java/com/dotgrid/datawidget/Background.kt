 ...
 * The work behind all of them is a [android.app.usage.NetworkStatsManager]
 * query, which is a binder call into a service that reads its own on-disk
 * history. It is usually a few milliseconds and occasionally is not, and one of
 * the times it is not is straight after boot, when everything on the device is
 * asking that service something at once.
 *
...
internal object Background {
    ...
    private val executor = Executors.newSingleThreadExecutor { runnable ->
        Thread(runnable, "data-widget").apply { isDaemon = true }
    }
