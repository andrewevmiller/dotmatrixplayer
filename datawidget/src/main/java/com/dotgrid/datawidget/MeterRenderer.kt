...
/**
 * The gauge: an arc of dots rather than a swept ring, so the meter speaks the
 * same dot-matrix vocabulary as the Ndot face the numbers are set in.
 *
 * The arc opens at the bottom. That is not decoration - the readout stacks three
 * lines inside the ring, and a closed circle would either crowd them or force
 * the type down a size. The gap gives the last line somewhere to sit and reads
 * as a dial while doing it.
 */
object MeterRenderer {
     ...
     * reads as a continuous scale at 110dp, sparse enough that each dot is still
     * a dot rather than a dashed line.
     */
     ...
     * @param fraction usage over the allowance, uncapped, or null for "no limit
     *   set" - which renders as an inert rail of dim dots instead of a fill,
     *   because there is nothing here to be a fraction of.
     */
    fun render(
        sizePx: Int,
        fraction: Float?,
        ...
        // Ceil, so any usage at all lights the first dot: a meter that reads
        // empty while bytes are moving is worse than one dot of overstatement.
        val filled = fraction?.let {
            min(DOT_COUNT, ceil((it.coerceAtLeast(0f) * DOT_COUNT).toDouble()).toInt())
        } ?: 0

        // Keyed on the dot count, not the fraction behind it. Usage creeps
